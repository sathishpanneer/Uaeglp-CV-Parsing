...
namespace Uaeglp.Web.Controllers
{
    ...
    public class EventController : ControllerBase
    {
        ...
        [HttpGet("get-events-by-month/{userId}/{month}/{year}/{language}/{forProfile}/{ispublic}", Name = "GetEventsByMonth")]
        public async Task<IActionResult> GetEventsByMonth(int userId, int month, int year, LanguageType language = LanguageType.AR, bool forProfile = false, bool ispublic = false)
        {
            var result = await _service.GetEventsByMonth(userId, month, year, language, forProfile, ispublic);
            return Ok(result);
        }
