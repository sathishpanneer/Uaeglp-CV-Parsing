...
namespace Uaeglp.Web.Controllers
{
    ...
    public class ActivityAndChallengeController : Controller
    {
        ...
        [HttpGet("activity/{profileId}/{categoryId}/{activityId}", Name = "GetActivityAsync")]
        public async Task<IActionResult> GetActivityAsync(int profileId, int categoryId, int activityId)
        {
            var data = await _service.GetActivityAsync(profileId, categoryId,activityId);
            return Ok(data);
        }
        ...
        [HttpGet("get-reference-number/{profileId}/{initiativeId}", Name = "GetReference")]
        public async Task<IActionResult> GetReferenceAsync(int profileId, int initiativeId)
        {
            var data = await _service.GetReferenceAsync(profileId, initiativeId);

            return Ok(data);
        }

        [HttpPost("answers", Name = "AddActivityAnswerAsync")]
        public async Task<IActionResult> AddActivityAnswerAsync([FromForm]ActivityAnswerViewModel model)
        {
        ...
        [HttpGet("delete-participant-activity/{profileId}/{activityId}", Name = "DeleteParticipantAsync")]
        public async Task<IActionResult> DeleteParticipantAsync(int profileId, int activityId)
        {
            var data = await _service.DeleteParticipantAsync(profileId, activityId);
